...
function dX = albopictus_equation_system__smooth_zdia(t, X, pars, data)
...
% dAg = 1*(z_dia * pars.gamma_Ab * Ab - ( mA + f_Ag(T) ) * Ag);
% dAo = 1*(f_Ag(T) * Ag - ( mA + pars.mu_r + z_dia * pars.gamma_Ao ) * Ao);

influx = 10;
if t < 365
    dAb = 1*(z_dia * ( pars.gamma_Aem * Aem + pars.gamma_Ao * Ao ) - ( mA + pars.mu_r + z_dia * pars.gamma_Ab ) * Ab) + influx;
    dAg = 1*(z_dia * pars.gamma_Ab * Ab - ( mA + f_Ag(T) ) * Ag) + influx;
    dAo = 1*(f_Ag(T) * Ag - ( mA + pars.mu_r + z_dia * pars.gamma_Ao ) * Ao) + influx;
else
    dAb = 1*(z_dia * ( pars.gamma_Aem * Aem + pars.gamma_Ao * Ao ) - ( mA + pars.mu_r + z_dia * pars.gamma_Ab ) * Ab);
    dAg = 1*(z_dia * pars.gamma_Ab * Ab - ( mA + f_Ag(T) ) * Ag);
    dAo = 1*(f_Ag(T) * Ag - ( mA + pars.mu_r + z_dia * pars.gamma_Ao ) * Ao);
